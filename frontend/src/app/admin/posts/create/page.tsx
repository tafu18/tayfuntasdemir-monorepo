...
export default function AdminPostCreate() {
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // Upload image immediately to backend
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Yazı başarıyla eklendi! Gönderi listesine yönlendiriliyorsunuz...');
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            Portfolyo siteniz için yeni bir blog yazısı oluşturun.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
          ...
          <div className="mt-1 flex flex-col sm:flex-row gap-4 items-center">
            ...
            <div className="flex-1 w-full space-y-2">
              <div className="relative">
                ...
                >
                  <Upload className="w-4 h-4" /> {imageUploading ? 'Yükleniyor...' : 'Görsel Seç & Yükle'}
                </label>
              </div>
              <p className="text-[11px] text-slate-400">
                Lütfen makaleniz için kapak resmi olabilecek kaliteli bir görsel yükleyin.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-slate-100 dark:border-zinc-800/80 pt-6 flex justify-end gap-3">
          ...
          >
            <Save className="w-4 h-4" /> {isLoading ? 'Kaydediliyor...' : 'Kaydet'}
          </button>
        </div>
      </form>
    </div>
