...
export default function InteractiveTerminal() {
  ...
  const processCommand = (raw: string, currentHist: typeof history) => {
    ...
    switch (cmd) {
      ...
      case 'yardim':
        ...
        output = `
          ...
          <table class="w-full max-w-lg text-left text-xs">
            <tr><td class="font-bold text-emerald-450 pr-4">about / kimim</td><td>Tayfun Taşdemir hakkında detaylı bilgi.</td></tr>
            <tr><td class="font-bold text-emerald-450 pr-4">posts / yazilar</td><td>En son yayınlanan 5 blog yazısını listeler.</td></tr>
            <tr><td class="font-bold text-emerald-450 pr-4">tools / araclar</td><td>Uygulama merkezindeki araçları listeler.</td></tr>
            <tr><td class="font-bold text-emerald-450 pr-4">contact / iletisim</td><td>Sosyal ağlar ve iletişim kanalları.</td></tr>
            <tr><td class="font-bold text-emerald-450 pr-4">ip</td><td>IP adresinizi sorgular.</td></tr>
            <tr><td class="font-bold text-emerald-450 pr-4">date / tarih</td><td>Güncel tarih ve saat bilgisini gösterir.</td></tr>
            <tr><td class="font-bold text-emerald-450 pr-4">quote / soz</td><td>Rastgele bir bilge sözü yazdırır.</td></tr>
            <tr><td class="font-bold text-emerald-450 pr-4">theme [tema]</td><td>Tema değiştirir. matrix, classic, ubuntu, dark, light</td></tr>
            <tr><td class="font-bold text-emerald-450 pr-4">home / anasayfa</td><td>Ana sayfaya yönlendirir.</td></tr>
            <tr><td class="font-bold text-emerald-450 pr-4">clear / temizle</td><td>Terminal ekranını temizler.</td></tr>
          </table>
        `;
        break;
      ...
      case 'kimim':
        isHtml = true;
        output = `
          <p><span class="font-bold text-emerald-400">Tayfun Taşdemir</span></p>
      ...
      case 'yazilar':
        isHtml = true;
        if (posts.length > 0) {
          let listStr = '<p class="font-bold text-emerald-400 mb-1">En Son Blog Yazıları:</p><ul class="list-disc list-inside space-y-1">';
          posts.forEach(p => {
      ...
      case 'araclar':
        isHtml = true;
        output = `
          <p class="font-bold text-emerald-400 mb-1">Sistemdeki Aktif Araçlar:</p>
      ...
      case 'iletisim':
        isHtml = true;
        output = `
          <p class="font-bold text-emerald-400 mb-1">İletişim Bilgileri:</p>
          <ul class="space-y-1">
  ...
  const getThemeClasses = () => {
    switch (theme) {
      case 'matrix':
        return 'bg-zinc-950 text-emerald-500 border-emerald-500/20';
      case 'classic':
